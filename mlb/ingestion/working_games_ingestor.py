...
def fetch_todays_games(target_date=None):
    ...
    if target_date:
        # Parse target date string - handle both MM-DD-YYYY and YYYY-MM-DD formats
        if isinstance(target_date, str):
            try:
                ...
                try:
                    ...
                    game_date = dt.datetime.strptime(target_date, '%m-%d-%Y').date()
                except ValueError:
                    raise ValueError(f"Date format not recognized: {target_date}. Use YYYY-MM-DD or MM-DD-YYYY")
        else:
