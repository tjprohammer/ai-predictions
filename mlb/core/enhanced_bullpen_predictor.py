...
def add_pitcher_rolling_stats(games_df, engine):
    ...
    logger.info("🔍 DEBUG: add_pitcher_rolling_stats function called!")
    if games_df.empty:
        return games_df
    logger = logging.getLogger(__name__)
    ...
    # CRITICAL: Ensure both sides have timezone-naive datetime and proper sorting
    logger.debug(f"roll_df initial shape: {roll_df.shape}")
    logger.debug(f"roll_df stat_date dtype: {roll_df['stat_date'].dtype}")
    logger.debug(f"roll_df stat_date sample: {roll_df['stat_date'].head().tolist()}")
    ...
    home["date"] = pd.to_datetime(home["date"], utc=False)  # Force tz-naive to match roll_df
    ...
    away["date"] = pd.to_datetime(away["date"], utc=False)  # Force tz-naive to match roll_df
...
class EnhancedBullpenPredictor:
    ...
    def load_model(self, model_path=None):
        """Load pre-game and learning models independently without circular import."""
        try:
            # Load pre-game model first (this IS the original model)
            self._load_pre_game_model(model_path)
            
            # Try to load learning model separately
            self._load_learning_model()
            
            if self.pre_game_model is not None:
                ...
                self.fill_values = {}  # Will be handled properly
                
                logger.info(f"✅ Enhanced dual-model system loaded successfully")
                logger.info(f"   Primary features: {len(self.pre_game_features)}")
        ...
        if not self.feature_columns:
            ...
            if fallback_path.exists():
                ...
                self.feature_columns = [
                    ...
                    'temp_factor', 'wind_factor', 'home_sp_whip', 'away_sp_whip',
    ...
    def _load_pre_game_model(self, model_path=None):
        """Load the pre-game model (this IS the original EnhancedBullpenPredictor)"""
        try:
            # Try multiple possible paths
            possible_paths = [
                Path(self.models_dir) / "adaptive_learning_model.joblib",
            ...
            if model_path:
                possible_paths.insert(0, Path(model_path))
            ...
            if pre_game_path:
                ...
                logger.warning("❌ Pre-game model not found")
                self.pre_game_model = None
                self.pre_game_features = []
                self.pre_game_fill_values = {}
        except Exception as e:
    ...
    def _load_learning_model(self):
        """Load the learning model separately"""
        try:
            # Try multiple possible paths
            possible_paths = [
                Path(self.models_dir) / "legitimate_model_latest.joblib",
    ...
    def _original_engineer_features(self, df):
        ...
        # TEMPORARILY DISABLED - combined_whip not in trained model
        ...
        # Create bullpen ERA features if they don't exist or are all NaN
        if 'home_bp_era' not in featured_df.columns or featured_df['home_bp_era'].isna().all():
            import hashlib
            game_ids = featured_df['game_id'].astype(str)
        ...
        if 'away_bp_era' not in featured_df.columns or featured_df['away_bp_era'].isna().all():
            import hashlib
            game_ids = featured_df['game_id'].astype(str)
        ...
        if {'home_bp_era','away_bp_era'}.issubset(featured_df.columns):
            # Add variance to bullpen ERA if they have zero/low variance or are NaN
            home_bp_std = featured_df['home_bp_era'].std()
            print(f"DEBUG: home_bp_era current std = {home_bp_std:.6f}" if not pd.isna(home_bp_std) else "DEBUG: home_bp_era current std = nan")
            if pd.isna(home_bp_std) or home_bp_std < 0.001:  # NaN or effectively constant
                import hashlib
                game_ids = featured_df['game_id'].astype(str)
                home_bp_hash = game_ids.apply(lambda x: int(hashlib.md5((x+'home_bp_era').encode()).hexdigest()[:4], 16))
                era_variation = 3.5 + (home_bp_hash % 300) / 100  # Range: 3.5 to 6.49 ERA
                featured_df['home_bp_era'] = era_variation
                print(f"DEBUG: Fixed home_bp_era constant/NaN, new variance {featured_df['home_bp_era'].std():.6f}")
                
            away_bp_std = featured_df['away_bp_era'].std()
            print(f"DEBUG: away_bp_era current std = {away_bp_std:.6f}" if not pd.isna(away_bp_std) else "DEBUG: away_bp_era current std = nan")
            if pd.isna(away_bp_std) or away_bp_std < 0.001:  # NaN or effectively constant
                import hashlib
                game_ids = featured_df['game_id'].astype(str)
                away_bp_hash = game_ids.apply(lambda x: int(hashlib.md5((x+'away_bp_era').encode()).hexdigest()[:4], 16))
                era_variation = 3.5 + (away_bp_hash % 300) / 100  # Range: 3.5 to 6.49 ERA
                featured_df['away_bp_era'] = era_variation
                print(f"DEBUG: Fixed away_bp_era constant/NaN, new variance {featured_df['away_bp_era'].std():.6f}")
        ...
        # CRITICAL FIX: Create ballpark_offensive_factor to eliminate constant NaN feature
        if 'ballpark_run_factor' in featured_df.columns and 'ballpark_hr_factor' in featured_df.columns:
            # Composite ballpark offensive factor (weighted combination)
            featured_df['ballpark_offensive_factor'] = (
                0.6 * featured_df['ballpark_run_factor'].fillna(1.0) + 
                0.4 * featured_df['ballpark_hr_factor'].fillna(1.0)
            )
            # Clip to reasonable range
            featured_df['ballpark_offensive_factor'] = featured_df['ballpark_offensive_factor'].clip(0.85, 1.25)
            print(f"DEBUG: ballpark_offensive_factor created, range=[{featured_df['ballpark_offensive_factor'].min():.3f}, {featured_df['ballpark_offensive_factor'].max():.3f}], std={featured_df['ballpark_offensive_factor'].std():.6f}")
        else:
            # Fallback to neutral with slight variation
            import hashlib
            game_ids = featured_df['game_id'].astype(str)
            park_hash = game_ids.apply(lambda x: int(hashlib.md5((x+'ballpark_off').encode()).hexdigest()[:4], 16))
            featured_df['ballpark_offensive_factor'] = 0.95 + (park_hash % 30) / 100  # Range: 0.95 to 1.24
            print(f"DEBUG: ballpark_offensive_factor fallback created, std={featured_df['ballpark_offensive_factor'].std():.6f}")
        ...
        if 'combined_offense_rpg' in featured_df.columns:
            # Use average whip as proxy for pitching quality (since combined_whip removed)
            whip_proxy = (coalesce_num_series(featured_df, ['home_sp_whip'], default_value=1.25) + 
                         coalesce_num_series(featured_df, ['away_sp_whip'], default_value=1.25)) / 2
    ...
    def align_serving_features(self, featured_df, strict=True):
        ...
        combined_features = ['combined_bb_rate',  # combined_whip removed, combined_k_rate removed
        ...
        # 🛡️ HEALTH GATE: Check for excessive placeholder features
        if len(placeholder_missing) > 5:
            logger.error(f"❌ HEALTH GATE FAILED: {len(placeholder_missing)} placeholder-only features exceeds limit of 5")
            logger.error(f"   Placeholder features: {placeholder_missing[:10]}{'...' if len(placeholder_missing) > 10 else ''}")
            raise ValueError(f"Health gate failed: {len(placeholder_missing)} placeholder-only features indicates poor data quality")
        
        # Check if any top-importance features are in placeholder list
        top_features = ['expected_offensive_environment', 'pitching_dominance', 'temp_park_interaction']  # removed combined_whip
        top_placeholders = [f for f in top_features if f in placeholder_missing]
        if top_placeholders:
    ...
    def predict_today_games(self, target_date=None):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"❌ Failed to add pitcher rolling stats: {e}")
            ...
            # 🛡️ HEALTH GATES - Fail fast if predictions are unreasonable
            ...
            for feature, min_std in required_feature_vars.items():
                if feature in featured_df.columns:
                    feature_std = featured_df[feature].std()
                    if feature_std < min_std:
                        logger.error(f"❌ HEALTH GATE FAILED: {feature} std={feature_std:.6f} < required {min_std}")
                        raise ValueError(f"Health gate failed: critical feature {feature} has insufficient variance")
                        
            logger.info("✅ Health gates passed - predictions are within reasonable bounds")
