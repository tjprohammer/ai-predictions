...
def test_build_windows_release_builds_seed_before_packaging(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: [build_windows_release.ROOT / "release" / "MLBPredictor-Windows-v0.9.0-beta-Setup.exe"],
    )
    monkeypatch.setattr(
        build_windows_release,
    ...
    assert sidecar_calls == [("MLBPredictor", "0.9.0-beta")]
...
def test_build_windows_release_passes_release_dir_to_installer(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        build_windows_release,
        "_detect_built_release_artifacts",
        lambda *_args, **_kwargs: [tmp_path / "release-output" / "MLBPredictor-Windows-v0.9.0-beta-Setup.exe"],
    )
    monkeypatch.setattr(
        build_windows_release,
        "_write_release_sidecars",
