...
from src.features.common import (
    ...
    offense_snapshot,
    park_snapshot,
    persist_first5_totals_features,
    pitcher_snapshot,
...
def _load_frames(start_date, end_date, settings):
    ...
    frames = {
        "games": query_df(
            """
            SELECT game_id, game_date, game_start_ts, status, home_team, away_team, total_runs_first5, season
            FROM games
        ...
        "markets": query_df(
            ...
            FROM game_markets
            WHERE game_date BETWEEN :start_date AND :end_date
              AND market_type = 'first_five_total'
            """,
            {"start_date": start_date, "end_date": end_date},
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build historical first-five totals feature rows")
    ...
    games = frames["games"]
    if games.empty:
        log.info("No games found for first-five totals feature build")
        return 0

    for frame_name in ("team_offense", "pitcher_starts", "player_batting"):
    ...
    feature_version = f"{settings.model_version_prefix}_first5_totals_core"
        ...
        rows.append(
            {
                ...
                "actual_total_runs_first5": game.total_runs_first5,
            }
        )
        row = rows[-1]
        missing = count_missing_fallbacks(row, FIRST5_TOTALS_CERTAINTY_KEY_FIELDS)
    ...
    validate_columns(
        feature_frame,
        FIRST5_TOTALS_META_COLUMNS + FIRST5_TOTALS_FEATURE_COLUMNS + [FIRST5_TOTALS_TARGET_COLUMN],
        "first5_totals",
    )
