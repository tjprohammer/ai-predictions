    ...
    try:
        ...
        for i, game in enumerate(games):
            ...
            if i == 2:  # After 3 games (L3)
                l3_era = (float(total_er) * 9.0) / float(total_ip) if total_ip > 0 else None
                logger.info(f"  → L3 ERA (manual): {l3_era:.2f}")
            elif i == 4:  # After 5 games (L5)
                l5_era = (float(total_er) * 9.0) / float(total_ip) if total_ip > 0 else None
        ...
        for window in [3, 5, 10]:
            rolling_data = calculate_rolling_era(pitcher_id, test_date, window, DATABASE_URL_PSYCOPG2)
