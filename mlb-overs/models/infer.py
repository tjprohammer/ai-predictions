...
def _era_from_rows(sub: pd.DataFrame) -> float | None:
    ...
    # SURGICAL FIX D: Handle IP parsing for legacy rows (5.2 → 5⅔)
    def ip_to_decimal(ip_str):
        if pd.isna(ip_str): 
            return np.nan
        whole, dot, frac = str(ip_str).partition('.')
        try:
            return float(whole) + (2/3 if frac=='2' else 1/3 if frac=='1' else 0.0 if frac else 0.0)
        except (ValueError, TypeError):
            return np.nan
    
    ip_series = sub.get("ip", pd.Series(dtype=float))
    if not ip_series.empty:
        ip = ip_series.apply(ip_to_decimal).fillna(0).sum()
    else:
        ip = 0
