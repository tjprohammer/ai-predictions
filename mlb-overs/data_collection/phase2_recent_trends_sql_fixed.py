#!/usr/bin/env python3
"""
Phase 2 Recent Trends Enhancement - SQL-Fixed Version
Adds L7/L14/L20 recent performance trends with corrected SQL queries.
"""
...
class RecentTrendsEnhancerFixed:
    """Recent trends collector with fixed SQL queries"""
    ...
    def phase2_recent_trends_enhancement(self):
        """Main enhancement method with fixed SQL queries"""
        ...
        try:
            # Get ALL games that need recent trends enhancement
            cursor.execute("""
            ...
            for i, (game_id, home_team, away_team, date, home_team_id, away_team_id) in enumerate(games_to_enhance):
                try:
                    # Get recent trends for both teams using fixed SQL
                    home_trends = self.get_team_recent_trends_fixed(cursor, home_team, date)
                    away_trends = self.get_team_recent_trends_fixed(cursor, away_team, date)
                    
                    # Get starting pitcher recent performance
                    home_sp_trends = self.get_sp_recent_performance_fixed(cursor, home_team, date)
                    away_sp_trends = self.get_sp_recent_performance_fixed(cursor, away_team, date)
    ...
    def get_team_recent_trends_fixed(self, cursor, team_name, game_date):
        """Get team's recent performance trends with fixed SQL queries"""
        try:
            # Get L7 runs scored/allowed - FIXED: removed ORDER BY from aggregate query
            cursor.execute("""
                SELECT 
            ...
            # Get L14 offensive hits for OPS estimation - FIXED: removed ORDER BY
            cursor.execute("""
                SELECT 
            ...
            # Get L20 runs scored/allowed for longer-term trends - FIXED: removed ORDER BY
            cursor.execute("""
                SELECT 
    ...
    def get_sp_recent_performance_fixed(self, cursor, team_name, game_date):
        """Get starting pitcher recent performance with fixed SQL"""
        try:
            # Get team's starting pitcher ERA from last 3 starts - FIXED: removed ORDER BY
            cursor.execute("""
                SELECT 
...
def main():
    ...
    logger.info("🚀 Starting Phase 2 Recent Trends Enhancement (SQL-Fixed Version)")
    logger.info("📋 Adding L7/L14/L20 recent performance trends with correct SQL queries")
