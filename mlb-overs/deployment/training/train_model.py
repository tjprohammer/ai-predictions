#!/usr/bin/env python3
"""
MLB Over/Under Model Training Script
====================================
Main training script for the MLB over/under prediction system.

Addresses training-production mismatches by:
...
Key Features:
- No more 69% missing ERA data (uses complete enhanced_games table)
- No more zero-value features corrupting training
- Same 125→44 feature pipeline used in both training and production
- Consistent feature engineering with production predictions
...
def engineer_production_features(df):
    ...
    # Temporarily change to parent directory for imports
    import os
    ...
    try:
        ...
        # Initialize predictor with proper engine access
        ...
        # For training, we'll use the raw features instead of aligning to an existing model
        ...
        print(f"✅ Feature preparation complete: {X_training.shape[1]} training features")
        
        return featured, X_training
...
def main():
    parser = argparse.ArgumentParser(description="MLB over/under model training")
    ...
    # Connect to database (will be reused for enhanced features)
    engine = create_engine(DB_URL)
    ...
    # 2. Engineer features using production pipeline (passing engine access)
