...
def test_basic_model():
    ...
    try:
        ...
        # Test WITHOUT the heavy bias corrections (new model should be calibrated)
        bias_correction = model_data.get('bias_correction', 0.0)
        final_prediction = raw_prediction + bias_correction
        
        print(f"\\n🎯 PREDICTION RESULTS (NEW CALIBRATED MODEL):")
        ...
        if 6.0 <= final_prediction <= 12.0:
            ...
            if abs(final_prediction - market_total) < 2.0:
                ...
                # Test multiple scenarios to see consistency
                print("\\n🔬 TESTING MULTIPLE SCENARIOS:")
                
                scenarios = [
                    {'name': 'High Offense', 'home_team_runs_pg': 5.5, 'away_team_runs_pg': 5.8},
                    {'name': 'Pitcher Duel', 'home_sp_era': 2.1, 'away_sp_era': 2.3},
                    {'name': 'Coors Field', 'ballpark_run_factor': 1.15}
                ]
                
                all_reasonable = True
                for scenario in scenarios:
                    test_features = full_features.copy()
                    test_features.update(scenario)
                    test_df = pd.DataFrame([test_features])[model.feature_names_in_]
                    
                    test_pred = model.predict(test_df)[0] + bias_correction
                    print(f"   {scenario['name']}: {test_pred:.1f} runs")
                    
                    if not (6.0 <= test_pred <= 12.0):
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\\n💡 SOLUTION: Remove heavy bias corrections and use new calibrated model")
    else:
